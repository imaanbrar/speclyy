...
**In scope**
- **Stripe dashboard:** add two recurring CAD prices to the existing **Speclyy Pro** product (test + live mode):
  - Monthly — **CA$49/mo**
  - Annual — **CA$468/yr** (displayed as "$39/mo CAD billed annually" — `$39 × 12 = $468`, exact match to the USD pattern of $29 × 12 = $348)
  - Note: if you prefer **CA$469/yr** you must accept that the implied monthly is $39.08 and the display rounds. Recommend $468 for math parity.
- **Env vars** — rename existing, add CAD pair:
  - `STRIPE_PRICE_ID_PRO_MONTHLY` → `STRIPE_PRICE_ID_PRO_MONTHLY_USD` (rename in Vercel + `.env.local.example`)
  - `STRIPE_PRICE_ID_PRO_ANNUAL` → `STRIPE_PRICE_ID_PRO_ANNUAL_USD`
  - `STRIPE_PRICE_ID_PRO_MONTHLY_CAD` (new)
  - `STRIPE_PRICE_ID_PRO_ANNUAL_CAD` (new)
...
- **Stripe Tax** — enable in dashboard, set `automatic_tax: { enabled: true }` on the Subscription create call. **Register Canadian tax IDs in Stripe before deploy** (federal GST/HST + applicable provincial — see "Tax compliance gate" below). This task is **not complete** until tax registrations are filed and the IDs are entered into Stripe Tax's Registrations panel.
- **Tax compliance gate (launch-blocking).** Decision: CAD launch must ship with tax collection live, not deferred. The business owner files:
  - **Federal GST/HST** with CRA — required before any CAD revenue is collected (do **not** wait for the $30k threshold; we register pre-emptively to avoid a back-tax remediation).
  - **QST** with Revenu Québec — required if any Québec customers will be charged.
  - **PST** with BC / SK / MB — required if customers in those provinces will be charged.
  - Once issued, paste each registration number into Stripe Tax → Registrations. Stripe then computes correct rates per province automatically.
  This is non-engineering work but blocks the deploy. Treat it like a database migration that has to land first.
...
- **The act of filing tax registrations with CRA / Revenu Québec / provincial agencies.** That paperwork is the business owner's job — engineering owns the Stripe Tax wiring. **However, deploying without filed registrations is forbidden** — see the compliance gate under "Implementation notes".
...
Scenario: BC customer sees GST + PST line items
  Given a CAD subscription with billing address in British Columbia
  And Stripe Tax has both the GST registration and the BC PST registration configured
  When the first invoice is generated
  Then the invoice line items include GST 5% and PST 7%
...
- Stay on the existing **Speclyy Pro** product. Do **not** create a new product.
- Add two prices in **test mode** first, then mirror in **live mode**:
  - `Speclyy Pro — Monthly CAD` — recurring, CAD, **CA$49.00**, `interval = month`.
  - `Speclyy Pro — Annual CAD` — recurring, CAD, **CA$468.00**, `interval = year`.
- Tag both prices with metadata `{ region: 'CA' }` for analytics convenience.
- Verify Customer Portal still has plan switching disabled — adding new prices to a portal-enabled product can silently re-enable it.
...
- Register Canadian tax IDs in Stripe Tax's Registrations panel (GST/HST first; provincial as needed).
- Set `automatic_tax: { enabled: true }` on Subscription create (shown above).
- Verify a test invoice for an Ontario billing address shows HST 13%; for BC, GST 5% + PST 7% (PST is collected separately if registered).
...
- **No new product in Stripe.** Reviewer confirms two new *prices* on the existing Speclyy Pro product, not a duplicate product. Per-region products break the Customer Portal model.
- **Env-var rename is breaking** — anyone with stale env vars will see runtime crashes from `process.env.STRIPE_PRICE_ID_PRO_MONTHLY_USD!` being undefined. Coordinate the rename + deploy carefully:
  1. Add new vars (USD + CAD) alongside the old ones.
  2. Deploy code that reads the new vars.
  3. Remove the old vars on the next deploy.
...
- **Tax registrations are deploy-blocking, not "coordinate at launch".** Decision: file CRA GST/HST + applicable provincial registrations and enter the IDs in Stripe Tax **before** the deploy that enables CAD pricing. Reviewer confirms: (a) Stripe Tax → Registrations panel shows live Canadian IDs (not test/empty), (b) a test invoice for an Ontario billing address shows HST 13% in the line items, (c) the PR description lists the registration numbers (or Stripe IDs referencing them). Do not approve the deploy if any of these are missing.
...
- **Manual (BC tax):** test invoice for a BC billing address shows GST 5% + PST 7% as separate line items.
...
- **Tax registrations are deploy-blocking.** CRA GST/HST + applicable provincial registrations must be filed and entered into Stripe Tax before this task ships — not deferred to "coordinate at launch". See "Tax compliance gate" under Implementation notes.
