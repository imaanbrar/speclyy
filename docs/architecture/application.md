...
`NEXT_PUBLIC_` prefix exposes the variable to the browser bundle. Everything else is server-only. `SUPABASE_SERVICE_ROLE_KEY` and `STRIPE_SECRET_KEY` must never appear in client-side code.
